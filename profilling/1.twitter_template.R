## Adapted from Stephen Turner: getting genetics Done
...
repoDir <- ("~/Documents/Github/TwitterAch/")
...
setwd(paste(repoDir, "PAG2015", sep=""))
...
twitterchivePlots <- function (filename=NULL) {
    ...
    with(byHour, barplot(tweets, names.arg=hour, col="black", las=1, cex.names=1.2, cex.axis=1.2, main=paste("Number of Tweets by Hour (central time)", paste("Term:", searchTerm), paste("Date:", Sys.Date()), sep="\n")))
    dev.off()
    ...
    par(mar=c(5,10,4,2))
    with(ht[order(ht$Freq), ], barplot(Freq, names=ht, horiz=T, col="black", las=1, cex.names=1.2, cex.axis=1.2, main=paste("Barplot of Top hashtags", paste("Term:", searchTerm), paste("Date:", Sys.Date()), sep="\n")))
    dev.off()
...
filelist <- list("../PAGXXIII.txt")
lapply(filelist, twitterchivePlots)


twitterchivePlots("PAGXXII.txt")
